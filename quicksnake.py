...
# init for the game
def init():
        global apple, body, clock, direction, gamePaused, gamePsdTxt
        global gameOvrTxt, gameRstrtTxt, gameRunning, head
        ...
        gameRstrtTxt = gameFontSml.render("Pressed \'r\' to restart", 1, (100, 100, 100))
...
def update():
        global apple, body, direction, gamePaused, gameRunning
    
        # handle game pausing on space bar
        if not(gamePaused and gameRunning):
...
def draw():
        global body, gameRunning
        
        screen.fill((150, 150, 150))
        
        if gamePaused:
                screen.blit(gamePsdTxt, (screenW/2 - gamePsdTxt.get_width()/2, 20))
        if gamePaused or not gameRunning:
                screen.blit(gameRstrtTxt, (screenW/2 - gameRstrtTxt.get_width()/2, 260))
        if gameRunning:
                for bodyPart in body:
